...
Хранилище артефактов моделей (joblib, метрики, отчеты обучения) обеспечивает версионирование и независимость инференса от процесса обучения. Позволяет обновлять модель без остановки сервиса и откатываться к предыдущей версии при необходимости.

* **PostgreSQL (Gateway Storage).**
Хранит данные о пользователях. Это обеспечивает надежное управление доступом и устойчивость к перезапуску сервисов. Использование реляционной базы гарантирует целостность и доступность данных.
...
Контейнеризированный деплой (Docker Compose) с разделением на сервисы и возможностью независимого масштабирования инференса.
...
* **Минусы выбора:**
    - Ограниченная автоматизация масштабирования на уровне инфраструктуры.
    В базовой конфигурации (Docker Compose) масштабирование выполняется вручную; для полноценного production требуется настройка autoscaling.
    - Необходимость дополнительной настройки балансировщика и сетевой инфраструктуры.
    Балансировщик нагрузки (например, Nginx) требует отдельной конфигурации: маршрутизации, health-check, SSL, что увеличивает сложность эксплуатации.
...
Каждый сервис работает в отдельном контейнере, что ограничивает влияние потенциальной компрометации одного компонента на остальные.
...
Артефакты моделей хранятся во внешнем S3-совместимом хранилище и не включаются в код сервисов. Это позволяет: разграничить доступ к данным и логике; управлять версиями моделей; обновлять модели без риска повреждения приложения.
...
Система не обрабатывает персональные данные (например, ФИО, контакты), что существенно снижает требования к защите и упрощает обеспечение безопасности.
...
* Хранение:
    - определяется объемом данных (модели, датасеты, результаты), который остается незначительным (~10–100 MB) и практически не влияет на общий бюджет.
...
| POST /segment                  | Клиент -> Gateway | Batch-инференс сегмента Top-N% пользователей с наибольшей вероятностью оттока, сохраняется в S3 |
...
Решение: валидация входных данных на уровне Gateway.
...
* недоступность Inference Service
Решение: использование retry-механизмов на уровне клиента или Gateway; логирование неуспешных запросов для последующего анализа.
