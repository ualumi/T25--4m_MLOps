...
Обучение модели бинарной классификации для расчета индивидуальной вероятности отсутствия транзакции ($p_i$) в целевом периоде (декабрь 2025 года).
...
**Данные и свалидация:**
| Компонент данных | Источник формирования | Функциональное назначение | Контроль качества (QA) |
| :--- | :--- | :--- | :--- |
| **Транзакционный поток** (`amount`, `date`) | Входной датасет `train_data.csv` | Генерация признаков активности и расчет временных агрегатов (30/60/90 дней) | Валидация типов данных, проверка хронологии событий, фильтрация аномальных сумм |
| **Вектор целевых меток** (`churn`) | Поле `churn` (исторический отток) | Формирование обучающего сигнала и расчет бизнес-метрик (Lift, $\Delta CR$) | Анализ дисбаланса классов, исключение утечки целевого признака (Target Leakage) |
...
| **Служебные параметры** (Split, Weights) | Процесс препроцессинга | Обеспечение репрезентативности выборок и настройка `is_unbalance=True` | Контроль Random State, исключение пересечения пользователей (User Overlap) между выборками | 

**Результат этапа.** 
Спроектированная денормализованная витрина данных, где сущность — `user_id`, а признаки — расчетные агрегаты активности пользователя.
