...
###### 1. Рост доли пользователей с транзакцией (снижение оттока)
...
X_i
```
— вектор признаков пользователя, сформированный на основе его транзакционной истории,
...
Таким образом, для каждого пользователя рассчитывается вероятность отсутствия транзакции в целевом периоде, после чего пользователи сортируются по убыванию этой вероятности и формируется сегмент повышенного риска.
Основная цель модели — корректно ранжировать пользователей по вероятности неактивности, чтобы в верхней части ранжирования концентрировались пользователи с наибольшим риском отсутствия транзакции.
модель прогнозирует вероятность отсутствия транзакции, что соответствует бизнес-задаче раннего выявления пользователей с риском оттока.
...
Единицей предсказания является пользователь.
