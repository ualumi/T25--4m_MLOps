...
Целью пилота является проверка того, что ML-модель действительно позволяет повысить эффективность удерживающих кампаний в целевом периоде за счёт увеличения доли пользователей, совершающих транзакции, при одновременном снижении стоимости удержания по сравнению с текущим подходом сегментации пользователей при помощи заранее заданных пороговых правил.

#### Дизайн пилота

- **Этап 1. Offline validation (1–2 недели)**
...
- **Этап 2. Offline scoring + online A/B evaluation (4–8 недель)**
    * скоринг пользователей рассчитывается заранее (batch) до запуска кампании;
    * сегменты фиксируются на момент старта:
    Пользовательская база случайным образом делится на группы:

     Control (50%) — сегмент формируется текущим правилом (например, отсутствие активности N дней);
     ML (Test) (50%) — сегмент формируется на основе ML-скоринга (Top-20% пользователей с максимальным риском).
    ...
    * далее проводится A/B тест на реальных пользователях;
...
где:
    ...
    Позволяет оценить вклад ML-модели относительно текущего подхода.

- **Экономическая метрика: CPRU**
    ...
    Позволяет измерить бизнес-эффект от внедрения ML-модели.
...
2.  **Концентрация целевой аудитории в сегменте (Lift в Top-20% $\ge 1.5$):** Подтверждение способности модели приоритизировать пользователей с максимальным риском оттока.
...
- Baseline-оценка:
    ...
    измерение времени обучения и инференса;
    оценка объёма обрабатываемых данных.
- Оценка MVP-модели (LightGBM):
    измерение времени обучения на полной выборке;
    оценка влияния количества признаков и деревьев на время расчёта;
    проверка масштабируемости инференса на всей пользовательской базе.
- Дополнительно учитываются особенности выбранного алгоритма:
использование LightGBM позволяет минимизировать затраты на обучение за счёт гистограммного метода построения деревьев. Обучение и инференс проводятся в batch-режиме, что исключает необходимость в GPU-инфраструктуре и соответствует выбранному режиму применения модели в бизнес-процессе.
...
#### Инфраструктурная подготовка
 ...
 - ормирование таблиц скоринга user_id – probability_of_inactivity – rank;
 - реализация логики разбиения на A/B группы;
 - настройка логирования и мониторинга расчётов.

**Технические артефакты:**
 - Сериализация:
модель и параметры препроцессинга сохраняются как единый пайплайн (joblib / pickle) для обеспечения воспроизводимости признаков и результатов инференса.
 - Пост-фильтрация:
реализован модуль исключения сотрудников и специальных счетов из итогового сегмента в соответствии с бизнес-ограничениями.
 - Формат выдачи:
...
* Вывод:
cистема построена с учетом минимизации и псевдонимизации ПДн, что снижает риск нарушения ФЗ-152. Для полного соответствия закону необходимо: внедрить шифрование передачи данных, управление ключами доступа, политику хранения/удаления данных.
