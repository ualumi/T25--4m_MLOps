...
- **Этап 1. Offline validation**
    ML-модель обучается и применяется к историческим данным (январь-ноябрь):
...
- Baseline-оценка:
    ...
    - время обучения: до нескольких минут на полной выборке;
    - потребление памяти: до 2–4 ГБ;
    - инференс: линейный по количеству пользователей, выполняется за минуты.
- Оценка MVP-модели (LightGBM):
    ...
    Ожидаемые характеристики:
    - время обучения: не более 2-х часов;
    - потребление памяти: порядка 8–16 ГБ;
    - инференс: выполняется в batch-режиме и масштабируется линейно по числу пользователей.
