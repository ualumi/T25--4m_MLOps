...
##### Бизнес-цель  
Снижение уровня оттока клиентов и увеличение доли пользователей, совершающих транзакции в целевом периоде (декабрь 2025 года), за счёт предварительного выявления пользователей с высокой вероятностью отсутствия транзакции на основе анализа их поведенческих и транзакционных характеристик за период с января по ноябрь 2025 года и последующего проведения адресных удерживающих мероприятий.
...
```math
AUC = P(P_{inactive} > P_{active})
```
$P_{inactive}$ — предсказанная вероятность для неактивных пользователей;
$P_{active}$ — предсказанная вероятность для активных пользователей;
$P(P_{inactive} > P_{active})$ — вероятность того, что модель присвоит более высокий score неактивному пользователю, чем активному.
**ROC-AUC ≥ 0.70** — модель корректно различает активных и неактивных пользователей;
...
Система должна обеспечивать расчёт индивидуальной вероятности отсутствия транзакции в целевом периоде (декабрь 2025 года) для каждого пользователя на основании его поведенческих и транзакционных характеристик за январь–ноябрь 2025 года.
...
выделение сегмента фиксированного размера (Top-20%);
